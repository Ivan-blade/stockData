...
total = len(hk_codes)
print(f"  → 从 stock_list 读取 {total} 只港股", flush=True)
...
with ThreadPoolExecutor(max_workers=3) as executor:
    ...
    for future in as_completed(futures):
        ...
        if len(batch) >= BATCH_SIZE:
            ...
            batch = []
        if done % 400 == 0:
            print(f"    → {done}/{total} 已完成 {written + len(batch)} 只, 错误 {len(errors)}", flush=True)
        time.sleep(0.3)
...
print(f"  ✅ 采集完成: {written}/{total} 只写入, {len(errors)} 只错误", flush=True)
...
print(f"\n=== 最终结果: 成功写入 {written} 只 / 总计 {total} 只 ===", flush=True)
