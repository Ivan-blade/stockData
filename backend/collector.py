...
def refresh_stock_list(verbose=True):
    """刷新全量股票清单（仅 upsert，不删除）

    所有交易所都通过 INSERT...ON DUPLICATE KEY UPDATE 更新：
    - 新增的代码自动入库
    - 已有的代码覆盖名称/交易所
    - 不删除任何旧数据（退市股留着不影响，历史快照可能引用）
    
    港股通过 get_hk_stock_codes 自动过滤掉衍生品/ETF/债券。
    """
    import time as _time
    now = datetime.now()

    if verbose:
        print("🚀 刷新股票清单（仅更新，不删除）...", flush=True)
    ...
    if verbose:
        print(f"  📊 A{total_a} + HK{total_hk} + US{total_us} = {total_a+total_hk+total_us} 只（仅新增/更新，未删除）")
